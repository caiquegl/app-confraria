...
export async function updateOwnProfile(
  ...
  formData.append("cpf", onlyDigits(payload.cpf));
  formData.append("bio", payload.bio.trim());
  formData.append("phone", onlyDigits(payload.phone));

  if (payload.avatarUri) {
    const extension = getFileExtension(payload.avatarUri);
...
function sendProfileRequest(params: {
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onerror = () => reject(new Error("Falha de conexão ao salvar perfil."));
    xhr.ontimeout = () => reject(new Error("Tempo esgotado ao salvar perfil."));
