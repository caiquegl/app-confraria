...
export async function fetchEventsDiscoverSections(): Promise<EventsDiscoverSections> {
  const { data } = await api.get<DiscoverSectionsResponse>(apiRoutes.events.discoverSections);
...
export async function fetchEventsDiscoverList(params: {
  ...
  const { data } = await api.get<PublicProfileEventListItem[]>(
    apiRoutes.events.discover(params.scope, params.category),
  );
