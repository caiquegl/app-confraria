...
import { fetchPlaceAutocomplete } from "../services/event-create.service";
...
export function EventPlaceAutocompleteField({
  label,
  onChange,
  placeholder,
  required = false,
  value,
}: EventPlaceAutocompleteFieldProps) {
  const [query, setQuery] = useState(value?.description ?? "");
  const [suggestions, setSuggestions] = useState<EventPlaceReference[]>([]);
  ...
  const handleSelectPlace = (place: EventPlaceReference) => {
  ...
  return (
    <View>
      ...
      <View style={styles.inputWrap}>
        <Ionicons color="#9CA3AF" name="location-outline" size={18} />
        <TextInput
